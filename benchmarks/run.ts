...
async function runBenchmark() {
    ...
    const structuredData = new TextEncoder().encode(JSON.stringify(
    ...
    for (const test of testCases) {
        ...
        console.log(`| ${name.padEnd(10)} | ${(originalSize / 1024 / 1024).toFixed(1)}MB | ${(lastCompressed.length / 1024 / 1024).toFixed(2)}MB | x${ratio} | ${compSpeed.toFixed(1)} MB/s | ${decompSpeed.toFixed(1)} MB/s |`);
    }
    console.log("");
}
