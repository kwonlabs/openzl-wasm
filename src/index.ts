...
export function getVersion(): string {
    if (!wasmModule) throw new Error("OpenZL module not initialized. Call init() or load() first.");
...
export function compress(data: Uint8Array): Uint8Array {
    if (!wasmModule) {
        throw new Error("OpenZL module not initialized. Call await OpenZL.load() first, or use OpenZL.compressAsync().");
    }
...
export function decompress(data: Uint8Array): Uint8Array {
    if (!wasmModule) {
        throw new Error("OpenZL module not initialized. Call await OpenZL.load() first, or use OpenZL.decompressAsync().");
    }
